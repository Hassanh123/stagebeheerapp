...
class StageController extends Controller
{
    ...
    public function choose(Stage $stage)
    {
        ...
        Notification::create([
            ...
            'message' => "Je keuze voor stage '{$stage->titel}' staat nu in behandeling. Wacht op goedkeuring van de beheerder.",
        ]);
    ...
    public function adminApprove(Stage $stage, Request $request)
    {
        ...
        if ($student) {
            Notification::updateOrCreate(
                ...
                [
                    'status' => 'goedgekeurd',
                    'message' => "Je keuze voor stage '{$stage->titel}' is goedgekeurd. Begeleider: {$teacher->naam}.",
                ]
            );
        }

        return back()->with('success', 'Stage goedgekeurd en gekoppeld aan docent.');
    }
    ...
    public function adminReject(Stage $stage)
    {
        ...
        if ($student) {
            Notification::updateOrCreate(
                ...
                [
                    'status' => 'afgekeurd',
                    'message' => "Je keuze voor stage '{$stage->titel}' is afgekeurd. Kies een nieuwe stage.",
                ]
            );
        ...
        return back()->with('error', 'Stage afgekeurd en weer beschikbaar voor andere studenten.');
    }
}
